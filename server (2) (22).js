...
app.post('/api/service-states', (req, res) => {
  ...
  for (const k of svcKeys) {
    if (states[k] !== undefined && ['auto','operational','outage'].includes(states[k]))
      cur[k] = states[k];
  }
  ...
  saveStates(cur);
  broadcast();   // ← push to status page instantly
...
// ── Status page — server-side rendered with initial data ──────
